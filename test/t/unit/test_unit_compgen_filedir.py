...
class TestUnitCompgenFiledir:
    ...
    def test_fallback_1(self, bash, functions):
        ...
        with bash_env_saved(bash) as bash_env:
            bash_env.write_variable("BASH_COMPLETION_FILEDIR_FALLBACK", "set")
            completion = assert_complete(bash, "g f", cwd="_filedir/ext")
            assert completion == sorted("ff.e2 foo/".split())

    def test_fallback_2(self, bash, functions):
        """Directory names not matching $cur should not be generated"""
        with bash_env_saved(bash) as bash_env:
            bash_env.write_variable("BASH_COMPLETION_FILEDIR_FALLBACK", "set")
            completion = assert_complete(bash, "g h", cwd="_filedir/ext")
            assert completion == "h.e2"
