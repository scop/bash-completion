...
@pytest.mark.bashcomp(cmd="xrandr", pre_cmds=("PATH=$PWD/xrandr:$PATH",))
class TestXrandr:
    ...
    def test_output(self, completion):
        assert completion == sorted("DP-0 DP-1 DP-2 DP-3 eDP-1-1 HDMI-0".split())
    ...
    def test_output_mode(self, completion):
        assert completion == sorted("1024x768 1280x1024 1280x800 1600x900 1920x1080 720x480 "
    ...
    def test_addmode_second(self, completion):
        assert completion == sorted("1024x576 1280x800 1440x900 320x200 432x243 640x350 700x450 800x450 928x696 "
    ...
    def test_orientation(self, completion):
        assert completion == sorted("normal inverted left right 0 1 2 3".split())
    ...
    def test_setmonitor_third(self, completion):
        assert completion == sorted("DP-0 DP-1 DP-2 DP-3 eDP-1-1 HDMI-0 none".split())
    ...
    @pytest.mark.complete("xrandr --setprovideroutputsource modesetting ", env=ENV)
    ...
    @pytest.mark.complete("xrandr --setprovideroffloadsink modesetting ", env=ENV)
