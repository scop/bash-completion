...
class TestTmux:
    ...
    @pytest.mark.parametrize(
        "other_commands",
        [
            r"foo \;",  # escaped in bash, not in tmux
            r"foo\;",
            r"foo \\\\\;",  # tmux: escaped backslash, unescaped semicolon
            r"foo\\\\\;",
            r"foo ';'",
            r"foo '\\;'",  # tmux: escaped backslash, unescaped semicolon
            r'foo ";"',
    ...
    @pytest.mark.parametrize(
        "semicolon_arg",
        [
            r"foo \\\;",  # escaped in bash and tmux
            r"foo\\\;",
            r"foo '\;'",
            r"foo'\;'",
            r"foo '\\\;'",  # tmux: escaped backslash, escaped semicolon
            r"foo'\\\;'",
            r'foo "\;"',  # tmux: escaped semicolon
            r'foo"\;"',
            r'foo "\\;"',  # tmux: escaped semicolon
            r'foo"\\;"',
        ],
    ...
    @pytest.mark.xfail(reason="_comp_cmd_tmux() not called at all")
    @pytest.mark.complete(r"tmux foo \; new-")
    def test_tmux_subsequent_command_name_with_prefix(self, completion):
        # TODO: figure out why _comp_cmd_tmux() isn't being called in this case
        assert "new-session" in completion
