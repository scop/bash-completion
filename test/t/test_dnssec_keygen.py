...
class TestDnssecKeygen:
    ...
    @pytest.mark.complete("dnssec-keygen -a ",
                          env=dict(PATH="$PWD/dnssec-keygen:$PATH"))
    ...
    @pytest.mark.complete("dnssec-keygen -T ",
                          env=dict(PATH="$PWD/dnssec-keygen:$PATH"))
    ...
    @pytest.mark.complete("dnssec-keygen -m ",
                          env=dict(PATH="$PWD/dnssec-keygen:$PATH"))
    def test_11(self, completion):
        assert completion == sorted(
            "usage trace record size mctx".split())
